...
def main():
    sub_folder = '002_single_gradient_sweep'
    path = '//file/e24/Projects/ReinhardLab/data_setup_nv1/190227_d06_several_gradient_sweeps/{}'.format(sub_folder)
    pulsed_file = 'pulsed.029.mat'
    ...
    pulse_form_files = [
        'analogue_data_ch1_part1.txt',
        'analogue_data_ch1_part2.txt',
        'analogue_data_ch2_part1.txt',
        'analogue_data_ch2_part2.txt'
    ]
    ...
    plt.close('all')
    plt.plot(taus, integrated_currents[::2])
    plt.savefig('{}/taus_vs_is.jpg'.format(sub_folder), dpi=300)

    xs = np.linspace(integrated_currents[0], integrated_currents[-1], 500)
    interp_zs = np.array([np.interp(xs, integrated_currents[::2], zs[0]), np.interp(xs, integrated_currents[::2], zs[1])])
